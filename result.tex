...
\begin{proof}
    ...
    \begin{align*}
        \exists z \in \clBall_{\frac{1}{k}}(\Pointt)\cap&\Gr\ \exists \lambda > 0\\
        &[z + (0, \lambda)\Ball_{\varepsilon_0}(\Vect)]\cap\Gr  = \emptyset.
    \end{align*}

    % фаза 2: да ограничим ламбда отдолу и да построим z
    Нека $\bar{E}_0 = \bar{E}$. За $k\geq1$ строим множества $\bar{E}_k\subseteq \bar{E}$, реални числа $\lambda_k > 0$ и измерими функции $z_k:\bar{E}_k\to\R^{2n}$ по следния начин:\\
    Нека имаме $\bar{E}_k$ за някое $k\geq 0$.
    Дефинираме
    \begin{align*}
        \Lambda_s=\{&t\in\bar{E}_k \mid \exists z\in \clBall_{\frac{1}{k+1}}(\Pointt)\cap\Gr\\
    ...
    За всяко $k\geq 1$ дефинираме $(\tilde{x}_k, \tilde{y}_k) \in\Univ$ като
    \[
