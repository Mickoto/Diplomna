\chapter{Главното доказателство}
...
\begin{proof}
    ...
    \]
    Наистина ако $(x,y)\in\Gr$, поради непрекъснатостта на $F(\cdot, t)$, съществува $\delta_n > 0$, т.ч. $F(x,t) \subseteq F(x', t) + \frac{1}{n}\bar{\Ball}$ за всяко $x'\in \Ball_{\delta_n}(x)$.
    Да изберем $q_n\in\Q^n$ т.ч. $\lVert q_n - x\rVert \leq \min\{\delta_n, \frac{1}{n}\}$ и $y_n\in F(q_n, t)$, т.ч. $\lVert y - y_n\rVert \leq \frac{1}{n}$.
    ...
    % \]
    Обратното включване следва директно от затвореността на $\Gr$.\\
...
\begin{proof}
    ...
    и сумарно
    \[
        \lVert (x, \dot{x}) - (\bar{x}, \dot{\bar{x}}) \rVert =
    ...
    \[
        d((x, \dot{x}), Q)\leq
        \lVert (x, \dot{x}) - (\bar{x}, \dot{\bar{x}}) \rVert \leq
