...
Многозначно изображение $F:A\rightrightarrows B$ е такова, че стойностите му са подмножества на $B$, т.е. $F:A\to\mathcal{P}(B)$.
...
\begin{definition}[Хаусдорфово разстояние]
    Нека $Z$ е метрично пространство и $C,D\subseteq Z$. Хаусдорфовото разстояние между $C$ и $D$ е
\[
    d_H(C,D) = \inf\{\varepsilon > 0  \mid D \subseteq C + \varepsilon\bar{\Ball}\ \&\ C \subseteq D+\varepsilon\bar{\Ball}\}
\]
Това разстояние дефинира псевдометрика върху $\mathcal{P}(Z)$.
\end{definition}

Сега като можем да мерим разстояние в $\mathcal{P}(Z)$, можем да дефинираме непрекъснатост и липшицовост както бихме очаквали.
\begin{definition}
...
Най-важното свойство на измеримите изображения е, че можем да им намираме измерими селекции, както е доказано в Твърдение 3.2 в \cite{DeimlingMulti} % и Теорема 8.1.3 в \cite{SetValued}
...
\begin{theorem}
    ...
    \end{enumerate}
\end{theorem}
Удобни ще ни бъдат също така Теорема % TODO: find
...
Една класическа лема, която се среща често при изучаването на диференциални уравнения е следната:
...
\begin{lemma}[Филипов] \label{Filippov}
    ...
    Да разгледаме диференциалното включване от първа секция:
    \[
        (1) \left|
        \begin{array}{ll}
        ...
        \right.
    \]
    Нека $x:[0,T]\to\R^n$ е абсолютно непрекъсната,\\
    $x(0)\in\bar{\Ball}_{\delta}(x_0)$ и $d(\dot{x}(t), F(x(t),t)) \leq p(t)$ п.н. за някое $p\in L^1([0,T])$.
    Тогава съществува $\tilde{x}$ - решение на (1), т.ч:
    \begin{enumerate}
...
\end{lemma}
\begin{proof}
    Нека $x_0 = x$. Ще построим редица $x_n$ приближаваща решение на (1).\\
    По дадено $x_n$, избираме измерима селекция $y_n$ на $F(x_n(\cdot), \cdot)$, т.ч.
    \[
    ...
    \]
    Така $\dot{x}_{n+1} = y_n$.\\
    Да разгледаме количеството $\psi_n(t) = |x_{n+1}(t)-x_n(t)|$. Имаме:
    \[
    ...
    \]
    за достатъчно големи $n$. Следователно $\{x_n(t)\}_{n\geq 1}$ е фундаментална и сходяща и значи $x_n$ клони към някаква функция $\tilde{x}:[0,T]\to\R^n$ поточково, като
    \small
    \[
    ...
    \[
        \lVert \tilde{y}(t) - \dot{x}(t)\rVert =
        \lim_{n\to\infty} \lVert \dot{x}_1(t) - \dot{x}(t)\rVert + \sum_{i=2}^n \lVert \dot{x}_i(t) - \dot{x}_{i-1}(t)\rVert \leq
    \]
    \[
        \lim_{n\to\infty} l(t)\rho_n(t) + p(t) \leq
        l(t)\varphi(t) + p(t)
    \]

    Също така за функциите $\dot{x}(t)$ е изпълнено
    \[
    ...
    \[
        ...
        \ \text{п.н.}
    \]
    което, заради затвореността на $F(\tilde{x}(t), t)$, влече $\tilde{y}(t)\in F(\tilde{x}(t), t)$ п.н. След евентуална промяна на $\tilde{y}$ върху множество с мярка 0 получаваме желаното.
...
Ние ще ползваме 2 геометрични характеризации на този конус, които могат да бъдат намерени в \cite{Treiman}.
...
\begin{definition}[Трансверзалност на множества]
...
\end{definition}
За нашите цели по-лесно за проверяване е следното условие:
...
Ценността на това свойство се изразява в следните дефиниция и твърдение:
\begin{definition}
    Нека $C$ и $D$ са затворени подмножества на банахово пространство $Z$ и $x_0\in C\cap D$.
    Казваме, че $C$ и $D$ имат \textcolor{red}{tangential intersection property} в $x_0$ за конусите $\Tangent_C(x_0)$ и $T_D(x_0)$, ако
    \[
        \Tangent_{C\cap D}(x_0) \supseteq T_C(x_0) \cap T_D(x_0).
    \]
\end{definition}
\begin{proposition} \label{TIP}
    Нека $C$ и $D$ са затворени подмножества на банахово пространство $Z$ и $x_0\in C\cap D$.
    Ако $C$ и $D$ са субтрансверзални в $x_0$, то те имат \textcolor{red}{tangential intersection property} в $x_0$ за конусите $\Tangent_C(x_0)$ и $T_D(x_0)$.
\end{proposition}
